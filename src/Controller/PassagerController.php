...
class PassagerController extends AbstractController
{
    ...
    public function showEvent(int $id, EventRepository $eventRepository, EventParticipationRepository $participationRepository, WeatherService $weatherService): Response
    {
        ...
        try {
            $lieu = $event->getLieu();
            if (empty($lieu)) {
                throw new \Exception('Le lieu de l\'événement n\'est pas défini');
            }
            
            // Debug: Afficher le lieu
            dump('Lieu de l\'événement: ' . $lieu);
            
            // Récupérer les données météo pour tous les utilisateurs
            $currentWeather = $weatherService->getCurrentWeather($lieu);
            
            // Debug: Afficher les données météo brutes
            dump('Données météo brutes:', $currentWeather);
            
            if (isset($currentWeather['error'])) {
                throw new \Exception($currentWeather['error']);
            }
            
            $weatherData = $weatherService->formatWeatherData($currentWeather);
            
            // Debug: Afficher les données météo formatées
            dump('Données météo formatées:', $weatherData);
            
            // Récupérer les prévisions pour les prochains jours
            $forecast = $weatherService->getForecast($lieu, 3);
            if (!isset($forecast['error'])) {
                $forecastData = $forecast;
            }
        } catch (\Exception $e) {
            // Debug: Afficher l'erreur
            dump('Erreur météo:', $e->getMessage());
    ...
    ): Response
    {
        ...
        // Envoyer un SMS de confirmation (si le service est disponible)
        try {
        ...
        // Rediriger vers la page de QR code de participation
