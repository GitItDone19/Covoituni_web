...
class ConducteurController extends AbstractController
{
    ...
    public function mesParticipationsEvenements(
    ...
    ): Response
    {
        ...
        // Récupérer les réservations en attente pour les annonces d'événements créées par ce conducteur
        $reservationsEnAttente = [];
        $annonceEventsQuery = $entityManager->createQueryBuilder()
            ->select('ae')
            ->from('App\Entity\AnnonceEvent', 'ae')
            ->where('ae.driverId = :driverId')
            ->setParameter('driverId', $user->getId())
            ->getQuery()
            ->getResult();
            
        $annonceEventIds = array_map(function($ae) { return $ae->getId(); }, $annonceEventsQuery);
        
        if (!empty($annonceEventIds)) {
            $pendingReservations = $reservationRepository->createQueryBuilder('r')
                ->where('r.annonceEvent IN (:annonceIds)')
                ->andWhere('r.status = :status')
                ->andWhere('r.type = :type')
                ->setParameter('annonceIds', $annonceEventIds)
            ...
            // Ajouter les informations utilisateur à chaque réservation
