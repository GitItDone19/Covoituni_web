...
use App\Service\CarbonInterfaceService;
...
class ConducteurController extends AbstractController
{
    ...
    public function addVoiture(
        ...
        if ($request->isMethod('POST')) {
            ...
            // Set features from checkboxes
            $car->setAc($request->request->has('ac'));
            $car->setAux($request->request->has('aux'));
            $car->setBluetooth($request->request->has('bluetooth'));
            $car->setBlackWindows($request->request->has('black_windows'));
            $car->setGps($request->request->has('gps'));
            $car->setHeatedSeats($request->request->has('heated_seats'));
            $car->setParkingSensors($request->request->has('parking_sensors'));
            $car->setRearCamera($request->request->has('rear_camera'));
            $car->setSunroof($request->request->has('sunroof'));
            $car->setUsbPort($request->request->has('usb_port'));
            $car->setLeatherSeats($request->request->has('leather_seats'));
            $car->setCruiseControl($request->request->has('cruise_control'));
            $car->setLaneAssist($request->request->has('lane_assist'));
            $car->setAutomaticTransmission($request->request->has('automatic_transmission'));
            $car->setManualTransmission($request->request->has('manual_transmission'));
            
            // Use CategoryMatcherService to assign the best matching category
            $matchResult = $categoryMatcher->assignBestMatchingCategory($car);
            
            if (!$matchResult) {
                // If no matching category found, create a default one
                $defaultCategory = new \App\Entity\Categorie();
                $defaultCategory->setNom('Standard');
                $defaultCategory->setDescription('Catégorie standard de véhicule');
                $entityManager->persist($defaultCategory);
                $car->setCategorie($defaultCategory);
            }
            ...
            $this->addFlash('success', sprintf(
                'Votre voiture a été ajoutée avec succès ! Elle a été assignée à la catégorie "%s".',
                $car->getCategorie()->getNom()
            ));
            return $this->redirectToRoute('app_conducteur_voiture');
        }
        
        // Get car makes from API for the form
    ...
    #[Route('/voiture/calculate-co2', name: 'app_conducteur_calculate_co2', methods: ['POST'])]
    public function calculateCo2(Request $request, EntityManagerInterface $entityManager, CarbonInterfaceService $carbonService): JsonResponse
    {
        try {
            $data = json_decode($request->getContent(), true);
            
            if (!isset($data['carId']) || !isset($data['kilometrage']) || !isset($data['year'])) {
            ...
            $voiture = $entityManager->getRepository(Voiture::class)->find($data['carId']);
            if (!$voiture) {
                return new JsonResponse([
                    'success' => false,
            ...
            // Calculate CO2 emissions using the Carbon Interface service
            $emissions = $carbonService->calculateEmissions(
                $voiture->getMarque(),
                $voiture->getModele(),
            ...
            // Create and save the CO2 emissions record
            $co2Emission = new Co2Emissions();
            $co2Emission->setVoiture($voiture);
            $co2Emission->setEmissions($emissions);
            $co2Emission->setKilometrage((int)$data['kilometrage']);
            $co2Emission->setYear((int)$data['year']);
